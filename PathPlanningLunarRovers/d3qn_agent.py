...
from d3qn_network import DuelingDQN, D3QNLoss, create_networks, hard_update, soft_update
from prioritized_replay import PrioritizedReplayBuffer
from astar import AStarPlanner, create_grid_map
...
class D3QNAgent:
    ...
    def __init__(self, use_per: bool = True, use_astar: bool = True):
        ...
        # 损失函数
        self.loss_fn = D3QNLoss(gamma=config.GAMMA)
        ...
        if use_per:
            ...
            from prioritized_replay import ReplayBuffer
        ...
        # 动作空间
        self.num_actions = config.ACTION_SPACE_SIZE
    ...
    def train_step(self) -> Optional[float]:
        ...
        """
        # 检查是否有足够的样本（使用预填充阈值）
        min_size = getattr(config, 'MIN_REPLAY_SIZE', config.BATCH_SIZE)
        ...
        if self.use_per:
            (states, actions, rewards, next_states, dones,
             additional_features, next_additional_features,
             weights, indices) = self.replay_buffer.sample(config.BATCH_SIZE)
        ...
        if states.dim() == 4 and states.size(-1) == config.DEPTH_FRAME_STACK:
        ...
        loss.backward()
        # 梯度裁剪（使用配置参数）
        grad_clip = getattr(config, 'GRAD_CLIP_NORM', 10.0)
        ...
        if config.USE_SOFT_UPDATE:
            ...
            if self.training_step % config.TARGET_UPDATE_FREQ == 0:
                soft_update(self.online_net, self.target_net, config.TAU)
        else:
    ...
    def update_learning_rate(self):
        ...
        if self.current_lr > config.LR_MIN:
    ...
    def load_model(self, path: str) -> Dict:
        ...
        self.epsilon = checkpoint.get('epsilon', config.EPSILON_MIN)
...
class HierarchicalAgent:
    ...
    def __init__(self):
        ...
        # 状态缓冲
        self.state_buffer = deque(maxlen=config.DEPTH_FRAME_STACK)
    ...
    def init_planner(self, obstacles: List[Tuple[float, float, float]] = None):
        ...
        # 初始化A*规划器
        self.astar_planner = AStarPlanner(grid_map, config.GRID_RESOLUTION)
